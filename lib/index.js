...
function EnvCmd (args) {
  return Promise.resolve().then(() => {
    ...
    if (fs.existsSync(rcFileLocation)) {
      parsedEnvSource = UseRCFile({ envFile: parsedArgs.envFile })
    } else {
      // Try to use a .env file
      parsedEnvSource = UseCmdLine({ envFile: parsedArgs.envFile, useFallback: parsedArgs.useFallback })
    }

    return Promise.resolve(parsedEnvSource).then((parsedEnv) => {
      let env
