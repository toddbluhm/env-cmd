...
parser.addArgument('envFile', { action: 'store', help: 'filename path with environments variables' })
parser.addArgument([ '-f', '--fallback' ], { action: 'store', help: 'fallback filename if can not find the specified one' })
parser.addArgument([ '-n', '--no-override' ], { action: 'storeTrue', help: 'no override existing process env vars' })
parser.addArgument('command', { action: 'store', help: 'command to execute' })
parser.addArgument('commandArgs', { nargs: "*", action: 'store', help: 'command arguments' })
...
function ParseArgs (args) {
  ...
  const parsed = parser.parseKnownArgs(args)
  const parsedArgs = parsed[0]
  const unknownArgs = parsed[1]

  return {
    envFile: parsedArgs.envFile,
    command: parsedArgs.command,
    commandArgs: [...parsedArgs.commandArgs, ...unknownArgs],
    noOverride: parsedArgs.no_override,
    useFallback: parsedArgs.fallback
  }
}
...
function UseCmdLine (options) {
  ...
  let file
  try {
    file = PropertiesReader(envFilePath, { encoding: 'utf8' })
  } catch (err) {
  ...
  if (!file && options.useFallback) {
    try {
      file = PropertiesReader(envFilePathDefault)
    } catch (e) {
